...
if not defined terminal (
    mode 130, 32 >nul 2>&1
    if exist "%SysPath%\spp\store_test\" mode 134, 32 >nul 2>&1
    %psc% "&{$W=$Host.UI.RawUI.WindowSize;$B=$Host.UI.RawUI.BufferSize;$W.Height=32;$B.Height=300;$Host.UI.RawUI.WindowSize=$W;$Host.UI.RawUI.BufferSize=$B;}" %nul% >nul 2>&1
)

title ACTIVACION OFFICE BY: PC MATIC %masver% >nul 2>&1

echo: >nul
echo COMENZANDO LA ACTIVACION... >nul
echo: >nul
echo: ######   #####   #     #    #    ####### ###  #####  >nul
echo: #     # #     #  ##   ##   # #      #     #  #     # >nul
echo: #     # #        # # # #  #   #     #     #  #       >nul
...
echo: #        #####   #     # #     #    #    ###  #####  >nul
echo: >nul
echo: EL MEJOR SERVICIO TECNICO PARA TU PC , LAPTOP E IMPRESORA >nul
echo: _________________________________________________________________ >nul

call :dk_chkmal >nul 2>&1

if not exist %SysPath%\sppsvc.exe (
    %eline% >nul 2>&1
    set fixes=%fixes% %mas%troubleshoot >nul 2>&1
    call :dk_color2 %Blue% "Help - " %_Yellow% " %mas%troubleshoot" >nul 2>&1
    goto dk_done
)
...
call :dk_color %_Green% "	       LUEGO DE LA ACTIVACION ESTA VENTANA SE CERRARA AUTOMATICAMENTE" >nul
echo: >nul

set "_serv=sppsvc Winmgmt" >nul 2>&1
...
@echo off
echo Activating Office... [C2R ^| %_version% %_AudienceData%^| %_oArch%] >nul 2>&1
...
@echo off
echo Adding a Registry to Prevent Banner [Successful] >nul 2>&1
)
)
...
if not defined error (
    call :dk_color %Green% "Office is permanently activated." >nul
    if defined ohub call :dk_color %Gray% "Office apps such as Word, Excel are activated, use them directly. Ignore 'Buy' button in Office dashboard app." >nul
    echo Help: %mas%troubleshoot >nul
) else (
    call :dk_color %Red% "Some errors were detected." >nul
    if not defined ierror if not defined showfix if not defined serv_cor if not defined serv_cste call :dk_color %Blue% "%_fixmsg%" >nul
    echo: >nul
    set fixes=%fixes% %mas%troubleshoot
    call :dk_color2 %Blue% "Help - " %_Yellow% " %mas%troubleshoot" >nul
)
...
if not defined ierror (
    echo Symlinking System's sppc.dll to ["%_hookPath%\sppcs.dll"] [Successful] >nul
    if defined exhook (
        echo Copying Custom %_hook% to ["%_hookPath%\sppc.dll"] [Successful] >nul
    ) else (
        echo Extracting Custom %_hook% to ["%_hookPath%\sppc.dll"] [Successful] >nul
    )
) else (
    set error=1
    call :dk_color %Red% "Installing Ohook [Failed to %ierror%]" >nul
    echo: >nul
    call :oh_checkapps >nul
    if defined checknames (
        call :dk_color %Blue% "Close [!checknames!] and try again." >nul
        call :dk_color %Blue% "If it is still not fixed, reboot your machine using the restart option and try again." >nul
    ) else (
        if /i not "%ierror%"=="Copy" call :dk_color %Blue% "Reboot your machine using the restart option and try again." >nul
        if /i "%ierror%"=="Copy" call :dk_color %Blue% "If you are using any third-party antivirus, check if it is blocking the script." >nul
    )
    echo: >nul
)
...
if not defined exhook if not defined ierror (
    if defined hasherror (
        set error=1
        set ierror=1
        call :dk_color %Red% "Modifying Hash of Custom %_hook% [Failed]" >nul
    ) else (
        echo Modifying Hash of Custom %_hook% [Successful] >nul
    )
)
...
@echo off
echo Adding SharedComputerLicensing Reg [Successful] [Needed on Server With Retail Office] >nul 2>&1
)
)
...
@echo off
echo Activating Office... [MSI ^| %_version% ^| %_oArch%] >nul 2>&1
...
if %counter% EQU 0 (
set error=1
call :dk_color %Red% "Checking User Accounts SID              [Not Found]" >nul 2>&1
exit /b
)

if %counter% GTR 10 (
call :dk_color %Gray% "Checking Total User Accounts            [%counter%]" >nul 2>&1
)
...
@echo off
echo Clearing Office License Blocks [Successfully cleared from all %counter% user accounts] >nul 2>&1
...
if defined o16c2r if defined officeact (
    if exist "%defdat%\NTUSER.DAT" (
        reg load HKU\%defname% "%defdat%\NTUSER.DAT" %nul% >nul 2>&1
        reg query HKU\%defname%\Software %nul% >nul 2>&1 && (
            reg add HKU\%defname%\Software\Microsoft\Office\16.0\Common\Licensing\Resiliency /v "TimeOfLastHeartbeatFailure" /t REG_SZ /d "2040-01-01T00:00:00Z" /f %nul% >nul 2>&1
        )
        reg unload HKU\%defname% %nul% >nul 2>&1
    )

    for %%# in (%_sidlist%) do (
        reg delete HKU\%%#\Software\Microsoft\Office\16.0\Common\Licensing\Resiliency /f %nul% >nul 2>&1
        reg add HKU\%%#\Software\Microsoft\Office\16.0\Common\Licensing\Resiliency /v "TimeOfLastHeartbeatFailure" /t REG_SZ /d "2040-01-01T00:00:00Z" /f %nul% >nul 2>&1
    )
    echo Adding Registry to Skip License Check [Successfully added to all %counter% & future new user accounts] >nul 2>&1
)
...
@echo off
echo Checking OS Info [%winos% ^| %fullbuild% ^| %osarch%] >nul 2>&1
exit /b
...
@echo off
echo Installing Generic Product Key %~1 [Successful] >nul 2>&1
) else (
...
echo:
%psc% "$job = Start-Job { (Get-WmiObject -Query 'SELECT * FROM %sps%').Version }; if (-not (Wait-Job $job -Timeout 30)) {write-host 'sppsvc is not working correctly. Help - %mas%troubleshoot'> $null 2>&1}"
exit /b
...
@echo off
echo Checking WPA Registry Count [%wpainfo%] >nul 2>&1
)
